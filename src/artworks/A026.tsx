...
const Artwork = (props) => {
  ...
  function generateBalls(p5) {
    for (let i = 0; i < nBalls; i++) generateBall(p5, i);
  }
  ...
  function generateImage(p5) {
    if (ballShapeMode === 0) generateCircleImage(p5);
    if (ballShapeMode === 1) generateRingImage(p5);
    if (ballShapeMode === 2) generateSquareImage(p5);
    if (ballShapeMode === 3) generateHyperbolicImage(p5);
  }
  ...
  function generateSquareImage(p5) {
    ...
    for (let x = 0; x <= radius; x++)
      for (let y = x; y <= radius; y++) {
        delta = x;
        if (renderDiscretely) {
          if (invertAlpha) thisColor = p5.color(0, 0, 0, (255 * (nSteps - 1 - (delta / radius) * nSteps)) / (nSteps - 1));
          else thisColor = p5.color(0, 0, 0, (255 * ((delta / radius) * nSteps + 1)) / nSteps);
        } else {
          if (invertAlpha) thisColor = p5.color(0, 0, 0, 255 * (1.0 - delta / radius));
          else thisColor = p5.color(0, 0, 0, 255 * (delta / radius));
        }
  ...
  function generateRingImage(p5) {
    ...
    for (let x = 0; x <= radius * 2; x++)
      for (let y = 0; y <= radius * 2; y++) {
        ...
        if (delta < radius) {
          if (renderDiscretely) {
            if (invertAlpha) thisColor = p5.color(0, 0, 0, 255.0 * ((2 * p5.abs(delta / radius - 0.5) * nSteps) / (nSteps - 1.0)));
            else thisColor = p5.color(0, 0, 0, 255.0 * ((nSteps + 1 - 2 * p5.abs(delta / radius - 0.5) * nSteps) / (nSteps + 0.0)));
          } else {
            if (invertAlpha) thisColor = p5.color(0, 0, 0, 255 * (2 * p5.abs(delta / radius - 0.5)));
            else thisColor = p5.color(0, 0, 0, 255 * (1 - 2 * p5.abs(delta / radius - 0.5)));
          }
          ballImage.set(x, y, thisColor);
        } else {
    ...
    for (let x = 0; x <= radius; x++)
      for (let y = 0; y <= radius; y++) {
        ...
        if (rSquared < radius * radius) {
          if (renderDiscretely) {
            if (invertAlpha) thisColor = p5.color(0, 0, 0, (255 * ((p5.sqrt(rSquared) / radius) * nSteps + 1)) / nSteps);
            else thisColor = p5.color(0, 0, 0, (255 * (nSteps - (p5.sqrt(rSquared) / radius) * nSteps)) / nSteps);
          } else {
            if (invertAlpha) thisColor = p5.color(0, 0, 0, 255 * (rSquared / (radius * radius)));
            else thisColor = p5.color(0, 0, 0, 255 * (1 - rSquared / (radius * radius)));
          }
          ballImage.set(x, y, thisColor);
          ballImage.set(2 * radius - x, y, thisColor);
          ballImage.set(2 * radius - x, 2 * radius - y, thisColor);
          ballImage.set(x, 2 * radius - y, thisColor);
        } else ballImage.set(x, y, p5.color(0, 0, 0, 0));
      }
  }
  ...
  function generateHyperbolicImage(p5) {
    ...
    for (let x = 0; x <= radius; x++)
      for (let y = 0; y <= radius; y++) {
        ...
        delta = delta / 2.0;
        if (renderDiscretely) {
          if (invertAlpha) thisColor = p5.color(0, 0, 0, (255 * (nSteps - 1 - delta * nSteps)) / (nSteps - 1));
          else thisColor = p5.color(0, 0, 0, (255 * (delta * nSteps + 1)) / nSteps);
        } else {
          if (invertAlpha) thisColor = p5.color(0, 0, 0, 255 * (1.0 - delta));
          else thisColor = p5.color(0, 0, 0, 255 * delta);
        }
        ballImage.set(x, y, thisColor);
        ballImage.set(y, x, thisColor);
        ballImage.set(2 * radius - x, y, thisColor);
  ...
  function moveBalls(p5) {
    ...
    if (movementMode === 0) {
      for (let i = 0; i < nBalls; i++) {
        if (balls[i][0] - radius <= 0 || balls[i][0] + radius >= p5.width) balls[i][2] = -balls[i][2];

        if (balls[i][1] - radius <= 0 || balls[i][1] + radius >= p5.height) balls[i][3] = -balls[i][3];

        balls[i][2] += p5.random(-0.1, 0.1);
        balls[i][3] += p5.random(-0.1, 0.1);
    ...
    if (movementMode === 1) {
      ...
      let theta = 0;
      for (let i = 0; i < nBalls; i++) {
        if (balls[i][0] < -2 * radius || balls[i][0] > p5.width + radius || balls[i][1] > p5.height + radius) {
          v += p5.random(-2, 2);
          theta = p5.random(-0.2, 0.2);
    ...
    if (movementMode === 2) {
      for (let i = 0; i < nBalls; i++) {
        if (balls[i][0] >= radius && balls[i][0] <= p5.width - radius && balls[i][1] >= radius && balls[i][1] <= p5.height - radius) {
          balls[i][2] += ((p5.width / 2 - balls[i][0]) / (p5.width / 2)) * p5.random(0.9, 1.1);
          balls[i][3] += ((p5.height / 2 - balls[i][1]) / (p5.height / 2)) * p5.random(0.9, 1.1);
          balls[i][0] += balls[i][2];
          balls[i][1] += balls[i][3];
        } else {
          balls[i][0] = p5.random(radius, p5.width - radius);
    ...
    if (movementMode === 3) {
      let rSquared = 0;
      let theta = 0;
      const vel0 = p5.max(p5.width, p5.height) / 150;
      let vel = vel0 + 0;
      for (let i = 0; i < nBalls; i++) {
        rSquared = (balls[i][0] - p5.width / 2) * (balls[i][0] - p5.width / 2) + (balls[i][1] - p5.height / 2) * (balls[i][1] - p5.height / 2);
        theta = p5.atan2(balls[i][1] - p5.height / 2, balls[i][0] - p5.width / 2);
        if (rSquared > p5.max(p5.width + radius, p5.height + radius) * p5.max(p5.width + radius, p5.height + radius) * 0.25) {
          balls[i][0] = p5.random(radius, p5.width - radius);
          balls[i][1] = p5.random(radius, p5.height - radius);
          balls[i][2] = 0;
          balls[i][3] = 0;
    ...
    if (movementMode === 4) {
      let rSquared = 0;
      let theta = 0;
      const rSquaredMax = (p5.width / 2 + radius) * (p5.width / 2 + radius);
      for (let i = 0; i < nBalls; i++) {
        rSquared = (balls[i][0] - p5.width / 2) * (balls[i][0] - p5.width / 2) + (balls[i][1] - p5.height / 2) * (balls[i][1] - p5.height / 2);
        theta = p5.atan2(balls[i][1] - p5.height / 2, balls[i][0] - p5.width / 2);
    ...
    if (movementMode === 5) {
      ...
      for (let i = 0; i < nBalls; i++) {
        for (let j = 0; j < nBalls; j++)
          if (i !== j) {
            rSquared = (balls[i][0] - balls[j][0]) * (balls[i][0] - balls[j][0]) + (balls[i][1] - balls[j][1]) * (balls[i][1] - balls[j][1]);
        ...
        balls[i][2] = balls[i][2] * 0.95;
        balls[i][3] = balls[i][3] * 0.95;
  ...
  const draw = (p5) => {
    ...
    if (applyThreshold) {
      ...
      for (let i = 0; i < p5.pixels.length; i++) {
        if (p5.red(p5.pixels[i]) === 255) p5.pixels[i] = backgroundColor;
        else p5.pixels[i] = ballColor;
      }
      p5.updatePixels();
    }
    if (blackBackground) p5.filter(p5.INVERT);
  };
