...
const Artwork = (props) => {
  ...
  let toggle;
  const customColour = ["#ff0054", "#cc2936", "#ffb7c4", "#44e5e7", "#028090", "#9cedeb", "#ffbc42", "#ff9505", "#fff3b0", "#E87D1E"];

  function form1(p5, x, y, d, c, s) {
    const r = d * 0.5;
    const da = p5.TWO_PI / c;
    p5.beginShape();
    for (let i = 0; i < c; i++) {
      for (let j = -s; j <= s; j++) {
        const a = da * (i + (j * 1) / s);
        const rr = (r / s) * (p5.abs(j) + 1);
  ...
  const draw = (p5) => {
    ...
    const x = p5.width / 2;
    const y = p5.height / 2;
    const d = p5.max(p5.height / 2, p5.width / 2);
    const c = p5.floor(p5.random(3, 22));
    const s = p5.floor(p5.random(1, 7));
    const cc = p5.floor(p5.random(1, 20));
