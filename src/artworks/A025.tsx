...
const Artwork = (props) => {
  ...
  class RorschachShape {
    constructor(p5, params) {
      ...
      this.xNoiseParameterOffset = p5.createVector(Math.random(), Math.random()).mult(1024);
      this.yNoiseParameterOffset = p5.createVector(Math.random(), Math.random()).mult(1024);
      this.noiseTime = 0;
    ...
    drawVertices(p5, yScaleFactor) {
      ...
      for (let i = 0; i < this.vertexCount; i += 1) {
        ...
        const noiseX =
          (2 * p5.noise(this.xNoiseParameterOffset.x + this.noiseDistanceScale * currentBasePositionX, this.xNoiseParameterOffset.y + this.noiseTime) - 1) *
          noiseMagnitude;
        const noiseY =
          (2 * p5.noise(this.yNoiseParameterOffset.x + this.noiseDistanceScale * currentBasePositionX, this.yNoiseParameterOffset.y + this.noiseTime) - 1) *
          noiseMagnitude;
    ...
    checkScreen(p5, absolutePositionX, absolutePositionY) {
      ...
      if (absolutePositionX < xMargin || absolutePositionX > p5.width - xMargin || absolutePositionY < yMargin || absolutePositionY > p5.height - yMargin) {
