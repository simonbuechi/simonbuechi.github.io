    ...
    drawShape(p5, x, y, l, c, hexNum, rad, s) {
      ...
      if (l > num) {
        for (let n = 0; n < hexNum; n++) {
          this.drawShape(
            p5,
            ((l * 3) / 4) * p5.cos((n * 2 * p5.PI) / hexNum),
            ((l * 3) / 4) * p5.sin((n * 2 * p5.PI) / hexNum),
            l / 4,
            p5.color((p5.mouseX * 255) / p5.width, (p5.mouseY * 255) / p5.height, this.pos * 255),
            1,
            0,
            s
          );
          this.drawShape(
            p5,
            l * (p5.sqrt(3) / 4) * p5.cos((n * 2 * p5.PI) / hexNum + p5.PI / 6),
            l * (p5.sqrt(3) / 4) * p5.sin((n * 2 * p5.PI) / hexNum + p5.PI / 6),
            l / 4,
            p5.color(this.posThree * 255, (p5.mouseX * 255) / p5.width, (p5.mouseY * 255) / p5.height),
            3,
            0,
            s
          );
          this.drawShape(
            p5,
            l * (p5.sqrt(3) / 2) * p5.cos((n * 2 * p5.PI) / hexNum + p5.PI / 6),
            l * (p5.sqrt(3) / 2) * p5.sin((n * 2 * p5.PI) / hexNum + p5.PI / 6),
            l / 4,
            p5.color((p5.mouseY * 255) / p5.height, this.posTwo * 255, (p5.mouseX * 255) / p5.width),
            3,
            0,
            s
          );
