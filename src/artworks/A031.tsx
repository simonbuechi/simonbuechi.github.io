...
const Artwork = (props) => {
  ...
  function separateGrid(p5, x, y, d) {
    const sepNum = p5.int(p5.random(1, 4));
    const w = d / sepNum;
  ...
  function reset(p5) {
    ...
    for (let i = 0; i < COLOR_NUMBER; i++) {
      colors.push(
        p5.color((baseHue + i * COLOR_VARIANCE) % 360, p5.int(p5.random(SATURATION_LOW, SATURATION_HIGH)), p5.int(p5.random(BRIGHTNESS_LOW, BRIGHTNESS_HIGH)))
      );
    }
  }
  ...
  const draw = (p5) => {
    const ww = p5.sqrt(p5.width * p5.width + p5.height * p5.height);
    p5.push();
